    ...
    <div ref={setNodeRef} className={wrapperClassName} style={style}>
      <div className={headerClassName}>
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-success font-bold text-[10px] mr-1.5">
...
// 可拖拽的表格行组件
const SortableTableRow = ({ copyToClipboard, forward, selectedIds, toggleSelect, getStrategyDisplay, handleServiceToggle, handleEdit, handleDelete, handleDiagnose, formatFlow }: any) => {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: forward.id });
  const style = { transform: CSS.Transform.toString(transform), transition: isDragging ? 'none' : transition, opacity: 0.6, zIndex: 50, position: isDragging ? ('relative' as const) : undefined, willChange: 'transform', backgroundColor: isDragging ? "var(--nextui-default-100)" : undefined };
  ...
  return (
    <TableRow key={forward.id} ref={setNodeRef} style={style as any}>
      ...
      <TableCell className={`whitespace-nowrap text-foreground ${rowBg}`}><span className="cursor-pointer hover:text-primary transition-colors" onClick={() => copyToClipboard(forward.name, "规则名")}>{forward.name}</span></TableCell>
      <TableCell className={rowBg}>
        <div className="flex items-center gap-1.5 overflow-hidden">
          ...
          <span className="text-sm font-mono font-medium cursor-pointer hover:bg-default-200/50 rounded px-1 transition-colors truncate max-w-[100px] inline-block" title={inAddrOnly} onClick={() => copyToClipboard(inAddrOnly, "入口地址")}>{inAddrOnly}</span>
        </div>
      </TableCell>

      <TableCell className={rowBg}>
        <span className="text-sm font-mono font-medium cursor-pointer hover:bg-default-200/50 rounded px-1 transition-colors" onClick={() => copyToClipboard(forward.inPort.toString(), "入口端口")}>{forward.inPort}</span>
      </TableCell>
      ...
      <TableCell className={rowBg}>
        <div className="flex items-center gap-1.5 overflow-hidden">
          ...
          <span className="text-sm font-mono font-medium cursor-pointer hover:bg-default-200/50 rounded px-1 transition-colors truncate max-w-[100px] inline-block" title={remoteAddrOnly} onClick={() => copyToClipboard(remoteAddrOnly, "落地地址")}>{remoteAddrOnly}</span>
          {forward.remoteAddr.includes(',') && <span className="text-primary-400 ml-0.5">...</span>}
        </div>
      </TableCell>

      <TableCell className={rowBg}>
        <span className="text-sm font-mono font-medium cursor-pointer hover:bg-default-200/50 rounded px-1 transition-colors" onClick={() => copyToClipboard(remotePortOnly, "落地端口")}>{remotePortOnly}</span>
      </TableCell>

      <TableCell className={`whitespace-nowrap ${rowBg}`}><span className="text-sm font-medium text-default-600 font-mono">{formatFlow(getForwardDisplayFlow(forward))}</span></TableCell>
    ...
    <TableRow key={forward.id} ref={setNodeRef} style={style as any}>
      ...
      <TableCell className={`whitespace-nowrap ${rowBg}`}>
        <div className="flex items-center">
          <span className="font-medium text-default-700 text-sm">{forward.tunnelName}</span>
          <span className="text-success font-bold text-[12px] ml-1.5">^{formatTunnelTrafficRatio(forward.tunnelTrafficRatio)}</span>
        </div>
      </TableCell>
                ...
                >
                  ...
                  >
                    ...
                    >
                      <TableHeader>
                        {true && (
                          <TableColumn className="whitespace-nowrap flex-shrink-0 w-14">
                            {/* @ts-ignore */}
                        ...
                        <TableColumn className="whitespace-nowrap flex-shrink-0 w-[120px] pl-4" align="left" >操作</TableColumn>
                      </TableHeader>
                      <TableBody
                        emptyContent="暂无规则配置"
                            ...
                            >
                              ...
                              >
                                {(() => {
                                  ...
                                  const isGroupIndeterminate = groupIds.some(id => selectedIds.has(id)) && !isGroupSelected;
                                  ...
                                  return (
                                    ...
                                    >
                                      <TableHeader>
                                        <TableColumn className={`whitespace-nowrap flex-shrink-0 ${FORWARD_GROUPED_TABLE_COLUMN_CLASS.select}`}>
